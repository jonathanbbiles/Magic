...
| `FEE_BPS_ROUND_TRIP` | `30` | Assumed Alpaca round-trip (default per `trade.js:116`). Models maker-maker fills under the default `ENTRY_LIMIT_PRICE_MODE=bid_plus_tick` execution mode (~15 bps × 2 legs). **Audit this against real fills before tuning anything else**: pull the last 14+ closed crypto trades from Alpaca Activity, compute `actual_fee_bps = (fee_usd / fill_notional_usd) × 10000` per leg, sum round-trip, set this env to the observed value. The `backtestEnvFallbacks` resolver (2026-05-18) wires the env into the dashboard auto-backtest so every signal's expectancy recomputes on `/debug/backtest?refresh=true`. Alpaca's crypto API has no `post_only` flag — `bid_plus_tick` is already the maximum maker enforcement; if observed fees are still > 35 bps, the only remedy is a venue change. |
