...
| `SIGNAL_VERSION` | *(unset → auto)* | Selects which entry signal the scan loop uses. **Default `auto`**: the runtime signal selector (`backend/modules/signalSelector.js`) picks `ols`, `multi_factor`, or `mean_reversion` based on the most recent backtest evidence. If no signal has cleared `SIGNAL_SELECTOR_MIN_BPS` (default `+3 bps avgNetBpsPerEntry` over the last 30 days), all entries are vetoed (skip reason: `backtest_veto_active`). Set to `ols`, `multi_factor`, or `mean_reversion` to operator-override and pin the signal — the veto still applies to a pinned signal unless `SIGNAL_SELECTOR_VETO_ENABLED=false`. **All three backtests run on every Render restart**; results at `meta.backtest`, `meta.backtestMf`, `meta.backtestMeanRev`; decision at `meta.signalSelector`. The OLS-specific gates (`slope_not_positive`, `net_edge_below_min`, `honest_ev_below_min`, `projected_below_gross_target`) are skipped when the active signal is `multi_factor` or `mean_reversion` — those signals' own factor votes replace them. Structural gates (drawdown, sizing, freshness, spread, vol-cap, HTF, recent-high) still apply to all signals. The mean-reversion signal is in `backend/modules/meanReversionSignal.js`: enters on volume-confirmed 1%+ capitulation drops with RSI<30, BTC decorrelated, deep-drop-guard clearing; targets half the drop. |
...
| Var | What it does |
| --- | --- |
| `ENTRY_UNIVERSE_MODE` | Default `dynamic` (Phase 1) — scanner trades **every** active Alpaca crypto pair (USD-quoted, ex-stablecoins) returned by `/v2/assets`, typically 30+ symbols. Wider universe = more mean-reversion triggers per scan. The stale-quote pruner still drops chronically-stale symbols at the per-symbol level, and the spread gate is tier-aware (`SPREAD_MAX_BPS_TIER1/2/3`) so tier-3 alts get appropriate caps. Set `ENTRY_UNIVERSE_MODE=configured` in Render env to revert to the 12 deep-liquidity primary pairs (`ENTRY_SYMBOLS_PRIMARY`) — useful when alt-quote staleness dominates and you want a known-clean book. |
