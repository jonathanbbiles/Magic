...
| `SIGNAL_SELECTOR_REALIZED_FLOOR_BPS` | `-10` | Realized `avgNetBps` below which new entries are halted. `−10` sits well past the ~0–2 bps Binance.US round-trip fee plus single-trade noise, so it only fires on a signal that is genuinely losing, not a marginal one. Raise toward `0` to be stricter; lower (e.g. `-30`) to tolerate more live bleed before halting. |
| `SIGNAL_SELECTOR_REALIZED_LOOKBACK_TRADES` | `50` | Window of most-recent closed trades (for the active signal) the realized average is computed over. Recency-weighted so a signal that has turned around can clear the veto without waiting for ancient losers to age out of the full file. |
