...
| `BTC_LEAD_LAG_SYMBOL_BLOCKLIST` | `AVAX/USD,LINK/USD,ADA/USD` | **(2026-06-29)** `btc_lead_lag` is the live-pinned signal and was net-**negative on all 7 of its live symbols** in the 2026-06-29 dashboard snapshot. The 3 worst by per-trade expectancy — AVAX −14.2, LINK −11.1, ADA −10.4 bps — are blocked to trim the structural losers without un-pinning the signal; the realized breaker stays the backstop for the rest. Closed trades from blocked symbols are dropped from the realized-veto window (same anti-deadlock treatment as the MR blocklists). Same two-point wiring as the MR blocklists (live getter `getBtcLeadLagSignalForPair` → `{ok:false, reason:'btc_lead_lag_symbol_blocklisted'}`; auto-backtest in `index.js`). |

The filter is applied at TWO points to keep the live engine and the selector's backtest in sync:
1. `getMeanReversionSignalForPair` / `getRangeMeanReversionSignalForPair` / `getBtcLeadLagSignalForPair` in `backend/trade.js` early-return `{ok: false, reason: '..._blocklisted'}` for blocked pairs (zero bars-fetched cost).
...
| `SIGNAL_SELECTOR_REALIZED_CADENCE_ADAPTIVE` | `true` | **Cadence-adaptive recovery window — safe-only (2026-06-29).** The static `SIGNAL_SELECTOR_REALIZED_MAX_AGE_MS` above encodes an assumption — "an actively-trading signal closes `minTrades` trades within 24h, so the clock only ever ages out a *halted* signal." For a **low-throughput** signal that is false: post-only `btc_lead_lag`'s low maker fill rate means it does **not** close `minTrades` trades in 24h, so the static window ages the sample out *faster than fresh fills arrive* and the breaker goes **blind** (`insufficient_sample`) on a bleeder — the exact failure the 2026-06-22 `minTrades` 10→6 cut patched around. When ON (default), the age-out window is extended to `max(maxAgeMs, minTrades × the signal's own measured median inter-trade interval)` so the sample can't drain before that many fresh trades could have replaced it. **Floored at `maxAgeMs`, so it only ever *lengthens* the window — recovery is never faster than the static clock, only slower for slow signals.** Strictly pro-safety: worst case it equals the static-24h behaviour. Surfaced at `meta.signalSelector.realizedVeto.cadenceMs` + `.effectiveMaxAgeMs`. Set `false` to restore the pure static-24h clock. **Not** the banned re-pin anti-pattern — it never loosens the floor or swaps the signal; it only keeps the breaker armed *longer* on slow signals. |
