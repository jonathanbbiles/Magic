...
After the primary 30-day run completes, an **A/B run** fires automatically with `signalTargetFraction=BACKTEST_AUTORUN_AB_FRACTION` (default `0.5`) so each restart gives you a side-by-side comparison vs the live setting (`signalTargetFraction=1.0`). The result lands at `meta.backtestAlt` — same shape as `meta.backtest`. Compare `overall.avgNetBpsPerEntry` between the two to see whether the live fraction is still the right call. Disable the A/B with `BACKTEST_AUTORUN_AB_ENABLED=false`.
