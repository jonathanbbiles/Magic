...
1. **Determine entry signal** — the active signal's per-symbol evaluator (code default pinned to `microstructure_5m` as of 2026-06-01; the bare-loop fallback when `SIGNAL_VERSION` is unset is `mean_reversion`; override with `SIGNAL_VERSION`).
