...
1. **Determine entry signal** — the active signal's per-symbol evaluator. Code default `SIGNAL_VERSION=''` (un-pinned 2026-06-02); the bare-loop fallback is `mean_reversion`. Override with `SIGNAL_VERSION` in Render env. The realized-expectancy circuit breaker is the sole halt authority and is NOT reset by re-pinning a signal.
...
| `SIGNAL_SELECTOR_REALIZED_MIN_TRADES` | `10` | Minimum realized-trade sample for the active signal before the realized veto can fire. Below this the realized average is too noisy to act on (mirrors the drift alerter's `minTrades`). |
