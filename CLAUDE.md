...
Defaults live in both `liveDefaults.js` (locked by `liveDefaults.test.js`) and the `readNumber/readBoolean` fallbacks in `trade.js`. Surfaced at `meta.signalSelector.realizedVeto` (`{ veto, reason, signalVersion, realizedAvgNetBps, sampleSize, floorBps, minTrades, lookbackTrades, maxAgeMs, agedOutCount, clearsOnClock, clearsAtMs, clearsInMs, agedTradesPending, evaluatedAt }`). **Clear ETA (2026-06-24):** when the veto is engaged, the pure `estimateRealizedVetoClear(recent, { minTrades, maxAgeMs, nowMs })` helper predicts WHEN the time-decay self-recovery clock would lift it if no new trade closes first — the trigger is the `(sampleSize − minTrades + 1)`-th oldest in-window trade aging out, so `clearsAtMs = triggerTs + maxAgeMs`. `clearsOnClock=false` (and null ETA) when the clock can't recover it: `maxAgeMs` disabled, or ≥ `minTrades` untimestamped trades that never age out — recovery then needs fresh fills that beat the floor. The dashboard 🛑 SAFETY BRAKE tile renders this as "Clears in ~Xh". **Hard Rule #4 compliance:** all five vars are read in `trade.js` and the veto is wired into the live entry path; `meta.signalSelector.realizedVeto` is the active diagnostic consumer. **When extending:** keep `evaluateRealizedVeto` pure and reuse `driftAlerter.selectRealizedTrades` — the realized veto and the drift alert MUST agree on which trades count, or the dashboard and the gate will tell different stories.
