...
Live crypto trading bot. As of 2026-05-21 the bot supports two venues, plus a paper venue added 2026-08-03, controlled by `EXECUTION_VENUE` env var:
- `EXECUTION_VENUE=alpaca` (legacy): all order placement, account queries, bars, and quotes route through Alpaca crypto. Historical posture.
...
- `EXECUTION_VENUE=paper` (2026-08-03): the **in-process paper broker** (`backend/modules/paperBroker.js`) fills a virtual portfolio against Binance.US's **live public prices** (reuses the binance_us DATA path — `IS_BINANCE_EXECUTION` is true for paper). Only the 8 order/account primitives are intercepted (paper before the binance branch in trade.js). No API keys, no funds; `validateEnv` exempts paper from live-only host guards. Zero-risk test-bed. State persists to `paper_broker_state.json`.

## 2026-08-09: chop/regime gate + per-signal breaker calibration (PAPER — steps 1-2 of GROWTH_PLAN)

Owner-approved 2026-08-09. Both changes ship to the **paper venue only**; `EXECUTION_VENUE` and `PORTFOLIO_SIZING_PCT` are untouched.

**1. `backend/modules/btcRegimeGate.js` — causal chop filter.** 53% of `trend_momentum`'s 1,377 backtested trades fire in chop and lose there (−169 bps/trade vs +1,136 mixed / +1,397 trending). The gate suppresses entries while BTC's **trailing** Kaufman efficiency ratio over 30 daily closes is below 0.30 — causal, no lookahead (the regime buckets in `validate_trend_momentum_long.js` use the *entry quarter's* ER, which includes future bars: valid diagnostic, invalid filter). Validated: +458 → **+872 bps/trade**, win 28.5% → 40.8%, PF 2.00 → 3.32, max DD −19.2% → −7.8%, Calmar 0.75 → 1.42.

**The counter-intuitive part, do not lose it:** the gate **does not raise return** at current sizing — it halves throughput, so CAGR falls 14.4% → 11.1%. It buys return *per unit of risk*. Spending that on higher sizing is a separate, later decision gated on a live sample. Wired once per scan in `scanAndEnter` (market-wide read) and applied per candidate via `rejectTrade('chop_regime_btc_er_low', …)`; reuses the TTL-cached BTC daily bars the signal already fetches (no extra API call). **Fails open** — unknown regime never suppresses. Surfaced at `meta.btcRegimeGate`. Env: `BTC_REGIME_GATE_ENABLED` (true), `BTC_REGIME_GATE_ER_WINDOW` (30), `BTC_REGIME_GATE_MIN_ER` (0.30).

**2. Per-signal realized-expectancy breaker floor — ⚠️ THIS LOOSENS THE HALT POSTURE.** Owner-authorized 2026-08-09 (satisfies Hard Rule #5). The −5 bps global floor is a *scalping* calibration; replayed through the real `evaluateRealizedVeto` over `trend_momentum`'s own 1,377-trade walk-forward it would have had the brake **engaged 58.9% of the time** — an off switch, not a brake, for a 28.5%-win-rate trend-follower whose loss strings are by design. New pure helper `signalSelector.resolveRealizedVetoConfig({signalVersion, base, perSignal})` merges a per-signal override over the global config, per field. **Only `trend_momentum` is overridden; every other signal keeps the −5 global.**

Floor chosen by a rule fixed *before* looking at live data: shallowest floor keeping the brake engaged <15% on the ungated stream while still firing before the worst observed 20-trade window (−1,181 bps). Measured halt rates — −5: 58.9%/36.4%, −400: 31.4%/12.1% (the plan's first estimate, too tight), **−600: 14.7%/3.5% (shipped)**, −800: 3.8%/0.0% (dead brake). Env: `SIGNAL_SELECTOR_REALIZED_FLOOR_BPS_TREND_MOMENTUM` (−600, must be ≤0), `..._MIN_TRADES_TREND_MOMENTUM` (20), `..._LOOKBACK_TRADES_TREND_MOMENTUM` (20). `meta.signalSelector.realizedVeto.perSignalOverrideApplied` makes the loosened posture visible.

**The boundary this is built on:** −600 is correct on the zero-risk paper venue, where a false halt costs the whole experiment. **If the venue is ever flipped paper → `binance_us`, re-confirming this floor must be a conscious decision** — for real capital the account-level `drawdownHalt` is the right catastrophe brake, not a per-trade expectancy floor. The brake is loosened, NOT removed (a −900 bps average still halts) and every recovery mechanism is unchanged. **Not the banned re-pin anti-pattern**: no signal swap, and the floor is pre-registered from the risk tail rather than moved in response to a halt. Never nudge it because it fired.

**Ship gate:** `node backend/scripts/validate_shipped_regime_and_breaker.js` replays the walk-forward through the SHIPPED modules + `liveDefaults` values and asserts 13 conditions (gate reproduces the plan's numbers; old floor false-halts >50%; new floor <15% ungated and <5% gated; brake still fires on a −900 bps bleed; brake is not dead). It fails the build if a default drifts.

## 2026-08-03: strategy rebuild — `trend_momentum` daily trend-follower (READ before touching the exit path)

Default signal flipped `btc_lead_lag → trend_momentum` (`backend/modules/trendMomentumSignal.js`). It is DAILY time-series trend-following: **entry** `close>SMA20 & SMA20>SMA50`; **exit** a **trailing MA-cross** (`close<SMA20`) implemented in `reconcileExits` via the pure `evaluateTrendMomentumExit` helper. The trailing exit is a **NEW exit mechanism, owner-authorized 2026-08-03 (Hard Rule #5)** and applies ONLY to `trend_momentum` positions — every other signal's exit posture is untouched. Backstops are deliberately far out (2000-bps fixed catastrophe stop bypassing vol-scaling, 90-day max-hold, ~2000-bps TP) so the trailing exit is the real exit. Validated on real Binance.US daily klines (`scripts/validate_trend_momentum.js`): **+414 bps/trade, PF 2.25, +69,909 bps total over 720d** vs buy&hold −2,752 bps/symbol — BUT regime-dependent (window thirds: +1441 / +37 / −261 bps; classic trend-following: prints in trends, whipsaws in chop). An intraday 1h fixed-bracket version was built + validated FIRST and LOST (−36.6 bps/trade); it was discarded, not shipped. **Do not "fix" trend_momentum by adding a tight stop or fixed TP — that reproduces the losing version; the trailing exit + wide backstops ARE the validated design.** Watch the realized-expectancy breaker (−10 floor, scalping-tuned) in paper: a low-win-rate trend-follower can trip it on a whipsaw string.
...
## 2026-06-08: Performance epoch — non-destructive "point 0" reset

`backend/modules/performanceEpoch.js` marks a reset timestamp + baseline equity so the dashboard shows the CURRENT strategy's real performance without the ~274 legacy mean-reversion trades polluting it. `PERFORMANCE_EPOCH_AT` (ISO, in liveDefaults, currently `2026-06-08T23:00:00Z`) is the source of truth; on boot `loadEpoch` reconciles config-vs-persisted (`data/performance_epoch.json`) and re-anchors on change. Baseline equity captured lazily (first finite reading after the epoch) via `ensureBaseline`, called where `latestEquity` is computed in index.js. `closedTradeStats.buildScorecard(limit, sinceMs)` gained an optional `sinceMs` filter + an `avgRealizedNetBps` field. Surfaced at `meta.performanceEpoch` `{ active, epochStartIso, baselineEquity, currentEquity, pnlUsd, pctChange, scorecard(since) }`, shown on the frontend 📍 SINCE RESET tile. **Non-destructive** — nothing deleted; `meta.scorecard` stays the all-time view. Reset again by bumping `PERFORMANCE_EPOCH_AT`.
...
| `SIGNAL_SELECTOR_REALIZED_MAX_AGE_MS` | `86400000` (24h) | **Self-recovery clock (2026-06-11).** Trades closed longer ago than this are dropped from the veto window. `0` disables it (count-only window, the pre-2026-06-11 behaviour). The module-level `maxAgeMs` default is `0`; the LIVE default in `liveDefaults.js` / `trade.js` is 24h. |
| `SIGNAL_SELECTOR_REALIZED_CADENCE_ADAPTIVE` | `true` | **Cadence-adaptive recovery window — safe-only (2026-06-29).** Replaces the static `maxAgeMs` with `max(maxAgeMs, minTrades × the signal's measured median inter-trade interval)` so a LOW-THROUGHPUT signal's sample can't age out faster than `minTrades` fresh fills could replace it — the "breaker went blind at low throughput" failure the 2026-06-22 `minTrades` 10→6 cut patched around (post-only `btc_lead_lag` does NOT close `minTrades` trades within 24h). **Floored at `maxAgeMs` → it can only ever lengthen the window, never recovers faster than the static clock.** Strictly pro-safety (worst case == static-24h). Module default `false` (pure-fn backward-compat); LIVE default `true`. Pure helper `computeMedianInterTradeMs(filtered)`; only engages when `maxAgeMs > 0`. **Not** the banned re-pin anti-pattern — never loosens the floor or swaps the signal; it keeps the breaker armed *longer* on slow signals. |

Defaults live in both `liveDefaults.js` (locked by `liveDefaults.test.js`) and the `readNumber/readBoolean` fallbacks in `trade.js`. Surfaced at `meta.signalSelector.realizedVeto` (`{ veto, reason, signalVersion, realizedAvgNetBps, sampleSize, floorBps, minTrades, lookbackTrades, maxAgeMs, cadenceAdaptive, cadenceMs, effectiveMaxAgeMs, agedOutCount, clearsOnClock, clearsAtMs, clearsInMs, agedTradesPending, evaluatedAt }`). **Clear ETA (2026-06-24):** when the veto is engaged, the pure `estimateRealizedVetoClear(recent, { minTrades, maxAgeMs, nowMs })` helper predicts WHEN the time-decay self-recovery clock would lift it if no new trade closes first — the trigger is the `(sampleSize − minTrades + 1)`-th oldest in-window trade aging out, so `clearsAtMs = triggerTs + effectiveMaxAgeMs` (the cadence-adaptive window when enabled, else the static `maxAgeMs`). `clearsOnClock=false` (and null ETA) when the clock can't recover it: `maxAgeMs` disabled, or ≥ `minTrades` untimestamped trades that never age out — recovery then needs fresh fills that beat the floor. The dashboard 🛑 SAFETY BRAKE tile renders this as "Clears in ~Xh". **Hard Rule #4 compliance:** all six vars are read in `trade.js` and the veto is wired into the live entry path; `meta.signalSelector.realizedVeto` is the active diagnostic consumer. **When extending:** keep `evaluateRealizedVeto` pure and reuse `driftAlerter.selectRealizedTrades` — the realized veto and the drift alert MUST agree on which trades count, or the dashboard and the gate will tell different stories.

**Self-recovery (2026-06-11) — the breaker can now un-deadlock itself.** A count-only window (`lookbackTrades`) is *frozen* while the veto holds the bot at zero trades: no new trades close, so the losing sample never refreshes and the bot sits halted indefinitely (observed live 2026-06-11: `btc_lead_lag` stuck at `realizedAvgNetBps -6.16` over a frozen 10-trade window after the taker→maker cutover — the stale sample was taker-era, a strategy no longer running). `SIGNAL_SELECTOR_REALIZED_MAX_AGE_MS` ages trades out by close-`ts`: once fewer than `minTrades` remain in-window the veto lifts as `insufficient_sample`, the bot re-probes at its (tiny `PORTFOLIO_SIZING_PCT`) size, and the breaker re-judges on FRESH fills — recovering if they clear the floor, re-halting within ~`minTrades` closes if they don't. This is the **time** analogue of the 2026-06-07 per-symbol-blocklist exclusion (which un-freezes the window in the symbol dimension). It does NOT weaken the live-active breaker: when the signal is trading, `minTrades` fresh closes accumulate in hours, well inside 24h, so the clock only ever bites a *halted* signal. A trade with a missing/unparseable `ts` is never aged out (kept in-window). **This is NOT the banned re-pin anti-pattern (#455/#456):** it does not swap the signal or move the floor to dodge the breaker — the breaker stays armed and simply re-tests stale evidence on a clock instead of trusting it forever.
...
`backend/modules/symbolBlocklist.js` exposes `parseSymbolBlocklist`, `readMrBlocklistsFromEnv`, `isMrPairBlocked`, `isPairBlocked`, and `readBtcLeadLagBlocklistFromEnv` (2026-06-29). Live engine reads via the `MR_BLOCKLISTS` + `btcLeadLag` blocklist sets at module load in `trade.js`; the auto-backtest in `index.js` reads the same env vars and passes `blockedSymbols: [...]` into `runBacktestAndStore` for the corresponding slot.
...
| `BTC_LEAD_LAG_SYMBOL_BLOCKLIST` | `AVAX/USD,LINK/USD,ADA/USD` | (2026-06-29) `btc_lead_lag` is the live-pinned signal and was net-NEGATIVE on all 7 of its live symbols (per-symbol grid: AVAX −14.2, LINK −11.1, ADA −10.4, XRP −8.9, DOGE −5.2, SOL −4.2, ETH −1.5 bps). The 3 worst are blocked to trim the structural losers without un-pinning; the realized breaker stays the backstop. Read via `readBtcLeadLagBlocklistFromEnv`, gated in `getBtcLeadLagSignalForPair` (`reason: btc_lead_lag_symbol_blocklisted`), excluded from the realized-veto window, and synced into the `btc_lead_lag` auto-backtest. |
