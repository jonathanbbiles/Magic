...
Modules:
- `backend/modules/binanceAuth.js` — HMAC-SHA256 query-string signer + public/signed request helpers. Reuses node:https (no new deps). `resolveCredentials` trims the key/secret (a Render-pasted secret with a trailing newline silently flips every signature → -1022 / HTTP 400). `signedRequest` aligns its timestamp to Binance server time via the public `/api/v3/time` endpoint (offset measured lazily, re-measured ≤ every 30 min) and self-heals a -1021 INVALID_TIMESTAMP by resyncing + retrying once — both guard against the clock-skew / whitespace failure modes that block a freshly-deployed `binance_us` cutover even when the key+secret+signature math are all correct. Best-effort: if the time-sync call fails the offset stays 0 (identical to pre-sync behaviour), so it can never make a working deploy worse. No new env var (pure correctness, nothing to tune).
