...
withEnv({}, () => {
  ...
  assert.equal(LIVE_CRITICAL_DEFAULTS.ENTRY_UNIVERSE_MAX_SYMBOLS, '');
  ...
  assert.equal(LIVE_CRITICAL_DEFAULTS.ENTRY_TIER3_MIN_PORTFOLIO_USD, '0');
  ...
  assert.equal(LIVE_CRITICAL_DEFAULTS.ENTRY_TAKE_PROFIT_BPS_TIER2, '160');
  assert.equal(LIVE_CRITICAL_DEFAULTS.STOP_LOSS_BPS, '55');
  assert.equal(LIVE_CRITICAL_DEFAULTS.MIN_PROB_TO_ENTER, '0.48');
  assert.equal(LIVE_CRITICAL_DEFAULTS.MIN_PROB_TO_ENTER_TIER1, '0.50');
  ...
  assert.equal(LIVE_CRITICAL_DEFAULTS.ORDERBOOK_SPARSE_STALE_QUOTE_TOLERANCE_MS, '15000');
  ...
  for (const sourceText of [envProduction, envLiveExample, envExample]) {
    ...
    assert.match(sourceText, /ENTRY_TIER3_MIN_PORTFOLIO_USD=0/);
    ...
    assert.match(sourceText, /ENTRY_TAKE_PROFIT_BPS=120/);
    assert.match(sourceText, /ENTRY_TAKE_PROFIT_BPS_TIER1=130/);
    assert.match(sourceText, /ENTRY_TAKE_PROFIT_BPS_TIER2=160/);
    assert.match(sourceText, /STOP_LOSS_BPS=55/);
    assert.match(sourceText, /MIN_PROB_TO_ENTER_TIER1=0.50/);
