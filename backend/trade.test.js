...
(() => {
  ...
  // silently re-starve daily entries. With the scalper-friendly defaults
  // (`TARGET_NET_PROFIT_BPS=15`, `ENTRY_SLIPPAGE_BPS=3`), `MIN_NET_EDGE_BPS=2`
  // means the gate requires `fillProbability >= 2 / 12 = ~0.17`, which is
  // subordinate to the slope-positive guard (fillProbability > 0.5). Raising
  // MIN above 2 without also raising TARGET will resurrect the small-target
  // rejection problem and starve scalp entries. See README "Strategy
  // economics" / `MIN_NET_EDGE_BPS`.
...
assert.ok(Object.prototype.hasOwnProperty.call(universeSnapshot, 'dynamicUniverseInitStartedAt'));
assert.ok(Object.prototype.hasOwnProperty.call(universeSnapshot, 'dynamicUniverseInitInProgress'));
assert.ok(Object.prototype.hasOwnProperty.call(universeSnapshot, 'entryScanBlockedBy'));
assert.ok(Object.prototype.hasOwnProperty.call(universeSnapshot, 'universeZeroReason'));
...
assert.ok(tradeSourceEarly.includes('for (let chunkIndex = 0; chunkIndex < scanSymbolChunks.length; chunkIndex += 1) {'));
assert.ok(tradeSourceEarly.includes('const scanChunkSymbols = scanSymbolChunks[chunkIndex] || [];'));
assert.ok(tradeSourceEarly.includes('prefetchResult = await prefetchEntryScanMarketData(scanChunkSymbols);'));
assert.ok(tradeSourceEarly.includes('for (const symbol of scanChunkSymbols) {'));
